...
let byName = {}; // creates a new object where the person's name is associated to each person's details
ancestry.forEach(function (child) {
    byName[child.name] = child;
});

function hasKnownMother(child) {
    return byName[child.mother] != null; // filter out child records where mothers do not have their own records
}

function motherAge(child) {
    return child.born - byName[child.mother].born; // age of mothers as array
}
...
const ageArray = ancestry.filter(hasKnownMother).map(motherAge);
